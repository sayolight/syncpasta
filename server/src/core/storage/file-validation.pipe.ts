...
export class FileValidationPipe implements PipeTransform {
    constructor(private readonly opts: FileValidationPipeOptions = {
        maxSize: 25 * 1024 * 1024,
        allowedMimeRegex: /^(image\/(png|jpe?g|webp|gif)|video\/(mp4|webm))$/i,
        allowedSignatures: ['image/png', 'image/jpeg', 'image/webp', 'image/gif', 'video/mp4', 'video/webm'],
    }) {
    }
    ...
    async transform(value: Express.Multer.File, metadata: ArgumentMetadata) {
        ...
        if (!magicBytes || !this.opts.allowedSignatures?.includes(magicBytes.mime)) {
